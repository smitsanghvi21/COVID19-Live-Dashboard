...
export function Section({ title, fill, action, items, rollup, separator, }) {
    const { value: expanded, toggle: toggleExpanded, setFalse: setExpandedFalse, } = useToggle(false);
    ...
    const itemsMarkup = items.map((item) => {
        const { onClick, label, subNavigationItems } = item, rest = __rest(item, ["onClick", "label", "subNavigationItems"]);
        const hasSubNavItems = subNavigationItems != null && subNavigationItems.length > 0;
        return (<Item {...rest} key={label} label={label} subNavigationItems={subNavigationItems} onClick={handleClick(onClick, hasSubNavItems)}/>);
    });
    ...
    const toggleRollup = rollup && items.length > rollup.after && (<div className={styles.ListItem} key="List Item">
      <button type="button" className={toggleClassName} onClick={toggleExpanded} aria-label={ariaLabel} testID="ToggleViewAll">
